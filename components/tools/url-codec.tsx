    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="tool-panel p-5 space-y-3">
          <h3 className="font-semibold text-sm">{mode === "encode" ? "Text / URL" : "Encoded URL"}</h3>
          <Textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder={mode === "encode" ? "Enter URL or text to encode..." : "Enter encoded URL to decode..."} className="min-h-[250px] font-mono text-sm" />
          <Button onClick={process} className="gap-1.5"><ArrowRightLeft className="w-4 h-4" /> {mode === "encode" ? "Encode" : "Decode"}</Button>
        </div>
        <div className="tool-panel p-5 space-y-3">
